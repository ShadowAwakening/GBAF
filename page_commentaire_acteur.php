...
$commentaires = $bdd->prepare('SELECT * FROM commentaires WHERE id_article = ? ORDER BY id DESC LIMIT 3');
...
<body>
    ...
    <p><?= $donnees_article['logo_article'] ?></p>
    <h1><?= $donnees_article['titre_article'] ?></h1>
    <p><?= $donnees_article['text_article'] ?></p>
    <hr>
    <!-- <i class="fas fa-thumbs-up"> bonjour</i> -->

    <!--    Like / Dislike -->
    <a href="like_dislike.php?t=1&id=<?= $getidentifiant ?>">J'aime</a> (<?= $likes ?>)
   <br />
   <a href="like_dislike.php?t=2&id=<?= $getidentifiant ?>">Je n'aime pas</a> (<?= $dislikes ?>)
